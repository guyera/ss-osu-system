...
class HICO_test_set(Dataset):
    ...
    def __init__(self, data_dir, split='test'):
        ...
        self.verb_trans = torch.from_numpy(
            pickle.load(open('../configs/verb_mapping.pkl', 'rb'), encoding='latin1')).float()
    ...
    def __getitem__(self, idx):
        ...
        with h5py.File(
                osp.join(self.data_dir, 'feature', self.split, str(int(self.db[im_id]['filename'][-10:-4])) + '.h5'),
                'r') as f:
    ...
    def collate_fn(self, data):
        key, spatial, sub_vec, obj_vec, uni_vec, labels_s, labels_ro, labels_r, labels_sro, shape, hdet, odet, obj_class = zip(
            *data)
