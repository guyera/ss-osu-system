...
class Ensemble:
    def __init__(self, nets, data_loader, top_k, num_obj_classes, num_subj_classes, num_verb_classes):
        if len(nets) < 2:
    ...
    def compute_features(self, is_training):
        features = []
        labels = []
    ...
    def get_top3_SVOs(self, is_training, verbose=True):
        with torch.no_grad():
            features, labels = self.compute_features(is_training)
