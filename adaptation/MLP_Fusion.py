...
class MLP_Fusion(nn.Module):
    ...
    def forward(self, x):
        ...
        # Since the anomaly score has already been pre-appended
        # to the feature vector, we grab the anomaly score
        # via the last element of the vector x.
        anom_scores = torch.reshape(x[:,len(x[0])-1], (len(x),1))

        x = F.relu(self.fc1(x))

        x_concat = torch.cat((x, anom_scores), dim=1)          
