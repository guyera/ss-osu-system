...
def score_test(
    ...
    for act in all_grd_truth_act_presence.columns:
        ...
        try:
            ...
            #     zero_division=0.0
            # )
            post_red_precision, post_red_rec, post_red_f1 = interpolated_prec_rec_f1(
                y_true=all_grd_truth_act_presence[act].iloc[total_pre_red_btn:start_test_phase], 
                y_pred_pre_retrain=all_pred_act_presence_yn_pre_retrain[act].iloc[total_pre_red_btn:start_test_phase], 
                y_pred_post_retrain=all_pred_act_presence_yn_post_retrain[act].iloc[total_pre_red_btn:start_test_phase]
            )
            test_post_red_precision, test_post_red_rec, test_post_red_f1, _ = metrics.precision_recall_fscore_support(
                ...
                all_pred_act_presence_yn_post_retrain[act].iloc[start_test_phase:],
                average='binary',
                zero_division=0.0
            )
    ...
    for spe in all_grd_truth_spe_presence.columns:
        ...
        try:
            ...
            # )
            post_red_precision, post_red_rec, post_red_f1 = interpolated_prec_rec_f1(
                y_true=all_grd_truth_spe_presence[spe].iloc[total_pre_red_btn:start_test_phase], 
                y_pred_pre_retrain=all_pred_spe_presence_yn_pre_retrain[spe].iloc[total_pre_red_btn:start_test_phase], 
                y_pred_post_retrain=all_pred_spe_presence_yn_post_retrain[spe].iloc[total_pre_red_btn:start_test_phase]
            )
            test_post_red_precision, test_post_red_rec, test_post_red_f1, _ = metrics.precision_recall_fscore_support(
                all_grd_truth_spe_presence[spe].iloc[start_test_phase:], 
                all_pred_spe_presence_yn_post_retrain[spe].iloc[start_test_phase:],
                average='binary',
                zero_division=0.0
            )
    ...
    for spe in all_grd_truth_spe_counts.columns:
        ...
        if num_post_red_img_w_spe[spe] > 0:
            ...
            post_red_cnt_abs_err = interpolated_species_count_error(
                all_grd_truth_spe_counts[spe].iloc[total_pre_red_btn:start_test_phase], 
                all_pred_spe_counts_pre_retrain[spe].iloc[total_pre_red_btn:start_test_phase], 
                all_pred_spe_counts_post_retrain[spe].iloc[total_pre_red_btn:start_test_phase]
            )
        ...
        if num_test_post_red_img_w_spe[spe] > 0:
            test_post_red_cnt_abs_err = species_count_error(
                ...
                all_pred_spe_counts_post_retrain[spe].iloc[start_test_phase:], 
                metric='AE'
            )
            ...
            if estimate_ci:
                test_post_red_count_ci = boostrap_conf_interval(
                    ...
                    y_pred=all_pred_spe_counts_post_retrain[spe].iloc[start_test_phase:].to_numpy(), 
                    metric_name='count_err', 
                    n_samples=nbr_samples_conf_int
                )
    ...
    post_red_avg_abs_err = interpolated_species_count_error(
        all_grd_truth_spe_counts.iloc[total_pre_red_btn:start_test_phase], 
        all_pred_spe_counts_pre_retrain.iloc[total_pre_red_btn:start_test_phase], 
        all_pred_spe_counts_post_retrain.iloc[total_pre_red_btn:start_test_phase]
    )

    test_post_red_avg_abs_err = species_count_error(
        all_grd_truth_spe_counts.iloc[start_test_phase:], 
        all_pred_spe_counts_post_retrain.iloc[start_test_phase:], 
        metric='MAE'
    )
    ...
    '''
    post_red_avg_abs_err_count_ci, post_red_boostrap_avg_abs_errs = -1, -1
    test_post_red_avg_abs_err_count_ci, test_post_red_boostrap_avg_abs_errs = boostrap_conf_interval(
        ...
        y_pred=all_pred_spe_counts_post_retrain.iloc[start_test_phase:], 
        metric_name='avg_count_err', 
        n_samples=nbr_samples_conf_int
    )
    ...
    if total_pre_red_btn < start_test_phase:
        ...
        '''
        post_red_spe_avg_auc_ci, post_red_boostrap_avg_aucs = -1, -1
        post_red_spe_avg_pre_rec_f1_ci, post_red_boostrap_avg_pre_rec_f1s = -1, -1

        # test phase
        test_post_red_per_spe_auc_arr = -1
        ...
        test_post_red_spe_avg_auc_ci, test_post_red_boostrap_avg_aucs = boostrap_conf_interval(
            ...
            y_pred=all_pred_spe_presence_post_retrain.iloc[start_test_phase:], 
            metric_name='avg_auc', 
            n_samples=nbr_samples_conf_int
        )
        '''

        test_post_red_spe_avg_auc_ci, test_post_red_boostrap_avg_aucs = -1, -1
        test_post_red_spe_avg_pre_rec_f1_ci, test_post_red_boostrap_avg_pre_rec_f1s = boostrap_conf_interval(
            y_true=all_grd_truth_spe_presence.iloc[start_test_phase:], 
            y_pred=all_pred_spe_presence_yn_post_retrain.iloc[start_test_phase:], 
            metric_name='avg_pre/rec/f1', 
            n_samples=nbr_samples_conf_int
        )
    else:
    ...
    agg_species_presence = {
        'pre_red_btn':{
            'avg_auc': {
                ...
                'boostrap_values': -1  # pre_red_boostrap_avg_aucs
            },
            'avg_precision': {
                'value': pre_red_spe_avg_pre,
                'ci': -1, #  pre_red_spe_avg_pre_rec_f1_ci['avg_precision'],
                'boostrap_values': -1  # pre_red_boostrap_avg_pre_rec_f1s['avg_precision']
            },
            'avg_recall': {
                'value': pre_red_spe_avg_rec,
                'ci': -1,  # pre_red_spe_avg_pre_rec_f1_ci['avg_recall'],
                'boostrap_values': -1  # pre_red_boostrap_avg_pre_rec_f1s['avg_recall']
            },
            'avg_f1_score': {
                'value': pre_red_spe_avg_f1,
                'ci': -1,  # pre_red_spe_avg_pre_rec_f1_ci['avg_f1_score'],
                'boostrap_values': -1  # pre_red_boostrap_avg_pre_rec_f1s['avg_f1_score']
            }
        },
        'post_red_btn':{
            'avg_auc': {
                'value': post_red_spe_avg_auc,
                'ci': -1,  # post_red_spe_avg_auc_ci,
                'boostrap_values': -1  # post_red_boostrap_avg_aucs
            },
            'avg_precision': {
                'value': post_red_spe_avg_pre,
                'ci': -1,  # post_red_spe_avg_pre_rec_f1_ci['avg_precision'],
                'boostrap_values': -1  # post_red_boostrap_avg_pre_rec_f1s['avg_precision']
            },
            'avg_recall': {
                'value': post_red_spe_avg_rec,
                'ci': -1,  # post_red_spe_avg_pre_rec_f1_ci['avg_recall'],
                'boostrap_values': -1  # post_red_boostrap_avg_pre_rec_f1s['avg_recall']
            },
            'avg_f1_score': {
                'value': post_red_spe_avg_f1,
                'ci': -1,  # post_red_spe_avg_pre_rec_f1_ci['avg_f1_score'],
                'boostrap_values': -1  # post_red_boostrap_avg_pre_rec_f1s['avg_f1_score']
            }
        },
        'test_post_red_btn':{
            'avg_auc': {
                'value': -1, # test_post_red_spe_avg_auc,
                'ci': -1,  # test_post_red_spe_avg_auc_ci,
                'boostrap_values': -1  # test_post_red_boostrap_avg_aucs
            },
            'avg_precision': {
                'value': test_post_red_spe_avg_pre,
                'ci': -1,  # test_post_red_spe_avg_pre_rec_f1_ci['avg_precision'],
                'boostrap_values': -1  # test_post_red_boostrap_avg_pre_rec_f1s['avg_precision']
            },
            'avg_recall': {
                'value': test_post_red_spe_avg_rec,
                'ci': -1,  # test_post_red_spe_avg_pre_rec_f1_ci['avg_recall'],
                'boostrap_values': -1  # test_post_red_boostrap_avg_pre_rec_f1s['avg_recall']
            },
            'avg_f1_score': {
                'value': test_post_red_spe_avg_f1,
                'ci': -1,  # test_post_red_spe_avg_pre_rec_f1_ci['avg_f1_score'],
                'boostrap_values': -1  # test_post_red_boostrap_avg_pre_rec_f1s['avg_f1_score']
            }
        }
    }
    ...
    '''
    pre_red_act_avg_auc_ci, pre_red_boostrap_avg_act_aucs = -1, -1
    ...
    if total_pre_red_btn < start_test_phase:
        ...
        post_red_act_avg_auc_ci, post_red_boostrap_avg_act_aucs = -1, -1
        post_red_act_avg_pre_rec_f1_ci, post_red_boostrap_avg_act_pre_rec_f1s = -1, -1

        test_post_red_per_act_auc_arr = -1
        ...
        test_post_red_act_avg_auc_ci, test_post_red_boostrap_avg_act_aucs = boostrap_conf_interval(
            ...
            y_pred=all_pred_act_presence_post_retrain.iloc[start_test_phase:], 
            metric_name='avg_auc', 
            n_samples=nbr_samples_conf_int
        )
        test_post_red_act_avg_pre_rec_f1_ci, test_post_red_boostrap_avg_act_pre_rec_f1s = boostrap_conf_interval(
            y_true=all_grd_truth_act_presence.iloc[start_test_phase:], 
            y_pred=all_pred_act_presence_yn_post_retrain.iloc[start_test_phase:], 
            metric_name='avg_pre/rec/f1', 
            n_samples=nbr_samples_conf_int
        )
    else:
    ...
    agg_activity_presence = {
        'pre_red_btn':{
            ...
            'avg_precision': {
                ...
                'ci':  -1,  # pre_red_act_avg_pre_rec_f1_ci['avg_precision'],
                'boostrap_values': -1  # pre_red_boostrap_avg_act_pre_rec_f1s['avg_precision']
            },
            'avg_recall': {
                'value': pre_red_act_avg_rec,
                'ci': -1,  # pre_red_act_avg_pre_rec_f1_ci['avg_recall'],
                'boostrap_values': -1  # pre_red_boostrap_avg_act_pre_rec_f1s['avg_recall']
            },
            'avg_f1_score': {
                'value': pre_red_act_avg_f1,
                'ci': -1,  # pre_red_act_avg_pre_rec_f1_ci['avg_f1_score'],
                'boostrap_values': -1  # pre_red_boostrap_avg_act_pre_rec_f1s['avg_f1_score']
            }
        },
        'post_red_btn':{
            'avg_auc': {
            ...
            'avg_precision': {
                ...
                'ci': -1,  # post_red_act_avg_pre_rec_f1_ci['avg_precision'],
                'boostrap_values': -1  # post_red_boostrap_avg_act_pre_rec_f1s['avg_precision']
            },
            'avg_recall': {
                'value': post_red_act_avg_rec,
                'ci': -1,  # post_red_act_avg_pre_rec_f1_ci['avg_recall'],
                'boostrap_values': -1  # post_red_boostrap_avg_act_pre_rec_f1s['avg_recall']
            },
            'avg_f1_score': {
                'value': post_red_act_avg_f1,
                'ci': -1,  # post_red_act_avg_pre_rec_f1_ci['avg_f1_score'],
                'boostrap_values': -1  # post_red_boostrap_avg_act_pre_rec_f1s['avg_f1_score']
            }
        },
        'test_post_red_btn':{
            'avg_auc': {
            ...
            'avg_precision': {
                ...
                'ci': -1,  # test_post_red_act_avg_pre_rec_f1_ci['avg_precision'],
                'boostrap_values': -1  # test_post_red_boostrap_avg_act_pre_rec_f1s['avg_precision']
            },
            'avg_recall': {
                'value': test_post_red_act_avg_rec,
                'ci': -1,  # test_post_red_act_avg_pre_rec_f1_ci['avg_recall'],
                'boostrap_values': -1  # test_post_red_boostrap_avg_act_pre_rec_f1s['avg_recall']
            },
            'avg_f1_score': {
                'value': test_post_red_act_avg_f1,
                'ci': -1,  # test_post_red_act_avg_pre_rec_f1_ci['avg_f1_score'],
                'boostrap_values': -1  # test_post_red_boostrap_avg_act_pre_rec_f1s['avg_f1_score']
            }
        }
    }
...
def score_tests(
    test_dir, sys_output_dir,sys_output_dir2, bboxes_dir, session_id, session_id2, class_file_reader, log_dir,
    ...
    for test_id in test_ids:
        ...
        # if 'OND' in test_id:
            ...
            if (sys_output_dir / f'{session_id}.{test_id}_detection.csv').exists():
                ...
                class_lines_post_retrain = open(sys_output_dir2 / f'{session_id2}.{test_id}_classification.csv').read().splitlines()
                print(f'{session_id}.{test_id}_detection.csv \n {session_id2}.{test_id}_classification.csv')
            else:
...
def main():
    ...
    p.add_argument("sys_output_root2", help="system output directory, parent of 'OND'")
    ...
    p.add_argument('--box_pred_dir2', default='./session/temp/logsDete10',
        help="dir containing the system\'s prediction of each box (predicted species and activity)")
    args = p.parse_args()
    ...
    sys_output_dir2 = Path(args.sys_output_root2) #/'OND'/'svo_classification'
    ...
    session_ids2 = set()
    for file in sys_output_dir2.iterdir():
        session_id2 = file.name.split('.')[0]
        session_ids2.add(session_id2)
    print(session_ids2)
    if len(session_ids2) > 1:
        raise Exception('More than one session id in results dir')
    session_id2 = list(session_ids2)[0]
    ...
    score_tests(
        test_dir, sys_output_dir,sys_output_dir2, bboxes_pred_dir, session_id, session_id2, ClassFileReader(), log_dir,
