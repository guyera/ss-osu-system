...
with open('annotation_counts_v4_2.txt', 'w') as f:

    for slice in slices:

        df = pd.read_csv(f'dataset_v4_2_{slice}.csv')
        ...
        # print counts
        df2 = df.copy()
        df2['svo'] = df2['subject_name'].astype(str) + '__' + df2['verb_name'].astype(str) + '__' + df2['object_name'].astype(str)
        # f.write('SVO')
        # f.write(df2['svo'])
        #df2 = df2.groupby(['subject_name', 'verb_name', 'object_name'])['svo'].nunique().reset_index()

        f.write(f"\n*** S and O Both Present ***\n")
        df_both_present = df2[(df2["subject_name"] != "absent") & (df2["object_name"] != "absent")]
        svos = df_both_present["svo"].value_counts()
        f.write(f"{svos.to_string()}\n")

        f.write(f"\n*** S missing ***\n")
        df_s_missing = df2[df2["subject_name"] == "absent"]
        svos = df_s_missing["svo"].value_counts()
        f.write(f"{svos.to_string()}\n")

        f.write(f"\n*** O missing ***\n")
        df_o_missing = df2[df2["object_name"] == "absent"]
        svos = df_o_missing["svo"].value_counts()
        f.write(f"{svos.to_string()}\n")
