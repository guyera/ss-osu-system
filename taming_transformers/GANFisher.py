...
class ImageDataset(Dataset):
    def __init__(self, csv_file, root_dir, transform=None):
        ...
        self.image_paths = self.csv_read['new_image_path'].values
...
class CycleGAN:
    def __init__(self, config_path, ckpt_path, num_of_new_generated_img = 50, G_lr=1e-5, D_lr=1e-4, beta1=0.9, beta2=0.999, lambda_cycle= 3., device='cuda'):
    ...
    def train(self, iterations):
        ...
        for idx in pbar:
            ...
            fake_img_Y, latent_codes, _  = self.G_XtoY(real_img_X)
            reconstructed_img_X, latent_codes, _  = self.G_YtoX(fake_img_Y)

            fake_img_X, latent_codes, _  = self.G_YtoX(real_img_Y)
            reconstructed_img_Y, latent_codes, _  = self.G_XtoY(fake_img_X)
            ...
            loss_G_X = self.criterion_GAN(pred_fake_X, torch.ones_like(pred_fake_X))
            pred_fake_Y = self.D_Y(fake_img_Y)
            loss_G_Y =  self.criterion_GAN(pred_fake_Y, torch.ones_like(pred_fake_Y)) 
            ...
            cycle_loss_Y = self.criterion_cycle(reconstructed_img_Y, real_img_Y) * self.lambda_cycle 

            loss_cycle = cycle_loss_X + cycle_loss_Y 
            cycle_loss_X_all.append(cycle_loss_X.item())
            cycle_loss_Y_all.append(cycle_loss_Y.item())
            loss_G = loss_G_X + loss_G_Y + cycle_loss_X + cycle_loss_Y
            loss_G.backward()
            ...
            D_Y_loss = (loss_D_real_Y + loss_D_fake_Y) 
            D_Y_loss.backward()
        ...
        with torch.set_grad_enabled(False):
            ...
            cp_save_dir = f'./session/temp/{current_time}_lambada_{self.lambda_cycle}/'
            ...
            while gen_count < self.num_of_new_generated_img:
                ...
                for i in range(len(new_row['new_image_path'])):
