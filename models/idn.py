...
class semihard_loss(nn.Module):
    ...
    def forward(self, score, label):
        ...
        pos = score_ * label
        neg = score_ * (1. - label)
        l_inter = torch.mean(torch.sum(neg, dim=1) / (torch.sum(1 - label, dim=1) + 1) - torch.sum(pos, dim=1) / (
                    torch.sum(label, dim=1) + 1))
        l_intra = torch.mean(torch.sum(neg, dim=0) / (torch.sum(1 - label, dim=0) + 1) - torch.sum(pos, dim=0) / (
                    torch.sum(label, dim=0) + 1))
...
class AE(nn.Module):
    def __init__(self, config, hoi_weight):
        ...
        for i in range(1, len(self.config.AE.SPATIAL.LAYER_SIZE)):
            self.spatial['fc%d' % i] = nn.Linear(self.config.AE.SPATIAL.LAYER_SIZE[i - 1],
                                                 self.config.AE.SPATIAL.LAYER_SIZE[i])
        ...
        for i in range(1, len(self.config.AE.ENCODER.LAYER_SIZE)):
            self.encoder['fc%d' % i] = nn.Linear(self.config.AE.ENCODER.LAYER_SIZE[i - 1],
                                                 self.config.AE.ENCODER.LAYER_SIZE[i])
        ...
        for i in reversed(range(1, len(self.config.AE.ENCODER.LAYER_SIZE))):
            self.decoder['fc%d' % (len(self.config.AE.ENCODER.LAYER_SIZE) - i)] = nn.Linear(
                self.config.AE.ENCODER.LAYER_SIZE[i], self.config.AE.ENCODER.LAYER_SIZE[i - 1])
            if i > 1:
                if self.config.AE.BN:
                    self.decoder['bn%d' % (len(self.config.AE.ENCODER.LAYER_SIZE) - i)] = nn.BatchNorm1d(
                        self.config.AE.ENCODER.LAYER_SIZE[i - 1])
                self.decoder['relu%d' % (len(self.config.AE.ENCODER.LAYER_SIZE) - i)] = nn.ReLU()
                if self.config.AE.DROPOUT:
                    self.decoder['dropout%d' % (len(self.config.AE.ENCODER.LAYER_SIZE) - i)] = nn.Dropout(
                        self.config.AE.DROPOUT)
    ...
    def forward(self, batch):
        ...
        if self.config.IDN.BINARY:
            ...
            if (len(batch['pos_ind']) > 0):
                L_bin_tran = self.classification_loss(tran_bin[batch['pos_ind'], :],
                                                      batch['labels_r'][batch['pos_ind'], :])
            else:
                L_bin_tran = 0
